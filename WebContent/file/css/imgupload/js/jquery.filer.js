...
(function($) {
						...
						if(s.get(0)
							...
							if(n.changeInput) {
								switch(typeof n.changeInput) {
									case "boolean":
										o = $('<div class="jFiler-input"><div class="jFiler-input-caption"><span>' + n.captions.feedback + '</span></div></div>"');
										break;
									case "string":
									case "object":
										o = $(n.changeInput);
	...
	$.fn.filer.defaults = {
		...
		captions: {
			...
			feedback2: "头像已经选中",
			drop: "删除上传的头像",
			removeConfirmation: "你确定要删除这个头像吗?",
			errors: {
				filesLimit: "仅有 {{fi-limit}}格式的图片可以上传.",
				filesType: "仅有图片可以上传.",
				filesSize: "{{fi-name}} 太大了，请上传小于 {{fi-maxSize}} MB 的文件.",
				filesSizeAll: "你上传的图片太大了，请上传小于{{fi-maxSize}} MB的图片."
			}
		}
	}
})(jQuery);